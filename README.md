...
* [`src/play_reframe/`](src/play_reframe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/play_reframe/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `play-reframe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/play_reframe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns play-reframe.example
  (:require [play-reframe.config :as config])
